...
# This will be fixed in the next update for the project, for now you get this barely functional slop.
# Don't try to fix this, it needs to be re-built from the groud up
...
# we're going to need to capture output to see if any errors occured
original_stdout = sys.stdout
captured_output = StringIO()

print("NSMBWer# build system for Linux\nBuilding XML:", sys.argv[1])
if sys.argv:
...
code_to_run = ""
all_build = ""
bindef =  "-externals=externals.txt -versions=versions-nsmbw.txt -output-kamek=build/\\$KV\\$.bin"

# check for meta data
meta = root.find("meta")
modules = root.find("modules")
specialRegion = root.find("kor")
...
Kmdef = "./Kamek "+all_build+version_km+" "+bindef
#print(Kmdef)
