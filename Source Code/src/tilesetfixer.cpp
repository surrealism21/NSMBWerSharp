...
// Main hook
inline void *TilesetFixerHack(void *cls, void *heap, int areaNum, int layerNum) {
	if (layerNum == 0) {
		for (int i = 1; i < 4; i++) {
			DoFixes(areaNum, i);
		}
...
kmCallDefCpp(0x80081718, void *, void *cls, void *heap, int areaNum, int layerNum) {
	return TilesetFixerHack(cls, heap, areaNum, layerNum);
}

kmCallDefCpp(0x800817C4, void *, void *cls, void *heap, int areaNum, int layerNum) {
	return TilesetFixerHack(cls, heap, areaNum, layerNum);
}
