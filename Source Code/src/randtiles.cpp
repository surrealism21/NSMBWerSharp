...
// Hooks
kmWrite32(0x80083598, 0x38600C3C);
kmWrite32(0x80086B4C, 0x48000120);
...
extern "C" bool RandTileLoadHook() {
	OSReport("Trying to load custom binaries...\n");
	void *buf = RandTileLoader.load("/RandTiles.bin");
	bool LIresult = LoadLevelInfo();
	if (buf == 0) {
		OSReport("Failed to load RandTiles.bin.\n");
		return false;
	} else {
...
// these are a pretty hacky way to get to the RandomTileData instance, but I don't care
kmBranchDefCpp(0x8015BCD0, NULL, bool, void) {
	return RandTileLoadHook();
}
...
kmCallDefCpp(0x800838AC, void, RTilemapClass *self) {
	IdentifyTilesets(self);
}
...
kmCallDefCpp(0x80086B48, void, RTilemapClass *self, BGRender *bgr) {
	TryAndRandomise(self, bgr);
}
