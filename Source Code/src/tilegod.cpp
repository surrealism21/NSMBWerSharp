...
kmBranchDefCpp(0x807E15D0, NULL, void, daChengeBlock_c* self, daChengeBlock_c::Action action, bool wasCalledOnCreation) {
//void daChengeBlock_c::doStuff(Action action, bool wasCalledOnCreation) {
	u16 perTilePatternFlag = 1, perRowPatternFlag = 1;

	u16 worldX = ((u16)self->pos.x) & 0xFFF0;
	u16 baseWorldX = worldX;
	u16 worldY = ((u16)(-self->pos.y)) & 0xFFF0;

	if (self->pattern == self->CheckerB) {
		perTilePatternFlag = 0;
		perRowPatternFlag = 0;
	}

	u16 tile;
	if (action != self->Destroy) {
		if (self->blockType & 8) {
			// Specify a tile number
			tile = 0x8000 | ((self->blockType & 3) << 8) | ((self->settings & 0xFF00000) >> 20);
		} else {
			// fall through
	...
	for (u16 y = 0; y < self->height; y++) {
		for (u16 x = 0; x < self->width; x++) {
			if (perTilePatternFlag) {
				u16 *pExistingTile = dBgGm_c::instance->getPointerToTile(worldX, worldY, self->currentLayerID);
				u16 existingTile = pExistingTile ? *pExistingTile : 0;

				dBgGm_c::instance->placeTile(worldX, worldY, self->currentLayerID, tile);
