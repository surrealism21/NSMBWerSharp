...
extern "C" u32 CXGetUncompressedSize(u8* sourceBuf);
extern "C" void CXUncompressLZ(void*, void*); // whatever
...
int dCreditsMgr_c::onCreate() {
	...
	if (!scriptLoader.load("/NewerStaffRoll.bin")) {
		OSReport("Could not load NewerStaffRoll.bin.\n");
		return false;
	}
	else {
		OSReport("Successfully loaded NewerStaffRoll.bin.\n");
	}
		...
		while (scriptPos && !exitInterpreter) {
			...
			switch (cmdType) {
				...
				case 7: { // Set names. FUN!
					...
					WriteUtf8ToTextBox(Title, (const char*)read);
					WriteUtf8ToTextBox(TitleS, (const char*)read);

					read += titleLength;

					WriteUtf8ToTextBox(Name, (const char*)read);
					WriteUtf8ToTextBox(NameS, (const char*)read);
