...
// Collision Callbacks
	...
	void dThunderCloud::yoshiCollision(ActivePhysics *apThis, ActivePhysics *apOther) {
		DamagePlayer(this, apThis, apOther);
	}

	bool dThunderCloud::collisionCatD_Drill(ActivePhysics *apThis, ActivePhysics *apOther) {
		this->playerCollision(apThis, apOther); return true;
	}
	...
	bool dThunderCloud::collisionCat5_Mario(ActivePhysics *apThis, ActivePhysics *apOther) {
		this->playerCollision(apThis, apOther); return true;
	}

	bool dThunderCloud::collisionCat11_PipeCannon(ActivePhysics *apThis, ActivePhysics *apOther) {
		this->playerCollision(apThis, apOther); return true;
	}
	...
	void dThunderCloud::setDeathInfo_IceBreak() {
		dEn_c::setDeathInfo_IceBreak();
		doStateChange(&StateID_DieFall);
	}

	void dThunderCloud::setDeathInfo_IceVanish() {
		dEn_c::setDeathInfo_IceVanish();
		doStateChange(&StateID_DieFall);
	}
...
static void lightningCallback(ActivePhysics *one, ActivePhysics *two) {
	if (one->owner->profileId == ProfileId::EN_THUNDERCLOUD && two->owner->profileId == ProfileId::EN_THUNDERCLOUD) {
		return; // to prevent the thundercloud from killing itself
	}
