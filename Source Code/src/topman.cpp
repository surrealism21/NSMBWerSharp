...
Profile TopmanProfile(&daTopman::build, SpriteId::EN_TOPMAN, &TopmanSpriteData, ProfileId::TARZAN_ROPE, ProfileId::EN_TOPMAN, "EN_TOPMAN", TMarcNameList, 0x200);
...
////////////////////////
	...
	void daTopman::setDeathInfo_IceBreak() {
		dEn_c::setDeathInfo_IceBreak();
		doStateChange(&StateID_Die);
	}
	void daTopman::setDeathInfo_IceVanish() {
...
///////////////
	...
	void daTopman::executeState_Wait() { 
		if (!this->isOutOfView()) {
			...
			PlaySoundWithFunctionB4(SoundRelatedClass, &handle, SE_BOSS_JR_CROWN_JR_RIDE, 1);
		}
