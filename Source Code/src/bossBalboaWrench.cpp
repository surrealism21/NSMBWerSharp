...
dActor_c *daBalboa_c::build() {
	...
	bool daBalboa_c::collisionCat7_GroundPoundYoshi(ActivePhysics *apThis, ActivePhysics *apOther) {
...
// Grow State
	...
	void daBalboa_c::beginState_ManholeUp() {
		...
		else if (randChoice == 3) {	// On the right ceiling!
			this->rot.y = 0xE000;
			this->rot.z = 0x8000;
			this->upsideDown = 1;
			this->direction = 0;
		...
		else if (randChoice == 4) {	// On the left ceiling!
			this->rot.y = 0x2000;
			this->rot.z = 0x8000;
			this->upsideDown = 1;
			this->direction = 1;
	...
	void daBalboa_c::executeState_Outro() {
		...
		if (ret == true) {
			...
			PlaySound(this, SE_EMY_CHOROPU_SIGN);
		}
